    ...
    axios.post(authApiEndpoints.login, data)
      ...
      .catch(error => {
        if (error.response && error.response.status === 422) {
          messagesRef.current?.show({ severity: 'error', detail: 'Incorrect email or password.', sticky: true });
        } else {
          messagesRef.current?.show({ severity: 'error', detail: 'Something went wrong. Try again.', sticky: true });
        }
        setSubmitting(false);
      });
  };
